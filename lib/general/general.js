    ...
    try {
        ...
        for (let palabra of palabras) {
            if (palabrasObject.oscuridad.includes(palabra.toLowerCase())) {
                await msg.reply.text(`La oscuridad se cierne sobre ${palabra}`);
                return;
            }
            ...
            if (palabrasObject.prohibidas.includes(palabra.toLowerCase())) {
                await msg.reply.text('Se tendrá en cuenta esa forma de hablar');
                if (u) {
                    u.advices += 0.10;
                    await u.save();
                    if (u.advices >= 3) {
            ...
            if(palabrasObject.izquierda.includes(palabra.toLowerCase())){
                await msg.reply.text(`Eso son cosas de rojos.`);
                return 0;
            }
            if(palabrasObject.perdon.includes(palabra.toLowerCase())){
                await msg.reply.text(`Perdonar es para los débiles o comunistas.`);
                return 0;
            }
        }
