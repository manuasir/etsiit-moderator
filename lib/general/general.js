    ...
    try {
        ...
        for (let palabra of palabras) {
            ...
            if(u && u.username === 'Zelopec'){
                msg.reply.text(getRandom(palabrasObject.zelopec,1)[0]);
                return 0;
            }
        ...
        if(randAnswer === 1){
            msg.reply.text(getRandom(palabrasObject.own,1)[0]);
        }
        return 0;
    } catch (err) {
        console.error(err);
            ...
            for(let item of menu){
                if(!item.includes('Pescado') && 
                    ...
                    !item.includes('Alérgenos')){
                        cleanedMenu.push(item.replace(',','\n'));
                }
            }
            msg.reply.text(cleanedMenu.toString());
        } else {
            msg.reply.text(menu.toString().replace(',','\n'));
        }
        return 0;
    } catch (err) {
    	console.error(err);
